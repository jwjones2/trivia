<?php
	 ...
	 * AddSponsor.php
	 *
	 * Description:  A form to add a new sponsor.  Self-posting.
	 * 	Inserts data for attendee and sponsor tables.
	 ********************************************************/
...
?>
    	  ...
    	  <?php
    	  	...
    	  	if ( isset($_POST['_submit_check']) && $_POST['_submit_check'] != 'getting_student_for_sponsor' )
    	  	{
        		/**
        		 ...
        		 * If successful, then print the New Sponsor form
        		 **/
        		...
        		$_POST['DateHolyGhost'] = build_date_from_form($_POST, 'dateHolyGhostYear', 'dateHolyGhostMonth', 'dateHolyGhostDay');
  		        $_POST['Anniversary'] = build_date_from_form($_POST, 'dateAnniversaryYear', 'dateAnniversaryMonth', 'dateAnniversaryDay');
        		...
        		// set value of SubDiscriminator for attendee type to be sponsor
        		$_POST['SubDiscriminator'] = "sponsor";
        		...
        		/**  
        		    NOW, Create a new Attendee and Sponsor object and then call
        		      functions to execute the bound query insertion into db.
        		 **/
        		$attendee = new Attendee($db, $_POST);
        		$sponsor = new Sponsor($db, $_POST);
        		...
        		// if query was successful then proceed to get the ID and insert sponsor
        		if ( $success )
        		{
        			// variable to check if any of the queries failed to handle the errors
        			$failed = false;  
        			
        			// set ID of sponsor from Attendee and insert Sponsor
        			// using the ID
        			$success = $sponsor->set_id_value($db->insert_id);
        			
        			// check that sponsor's ID was set with no errors
        			if ( $success ) 
        			{
        				$success = $sponsor->execute_bound_query();  // execute the query
        				
        				// check that query was successful and sponsor was entered
        				if ( $success ) 
        				{
        					/*** ATTENDEE AND SPONSOR ADDED SUCCESSFULLY!  REDIRECT TO SUCCESS PAGE ***/
        					/* Redirect browser */
 ...
 <!-- Form section -->
        <div id="formSection" class="formSection">
            <!-- top of form -->
            <span class="formTop center"><span class="moveDown">Add a New Sponsor to the Database</span></span>
			...
			<form enctype="multipart/form-data" id="attendeeForm" name="attendeeForm" class="attendeeForm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" onsubmit="return validateAttendeeForm()">
				<table cellpadding="3px">
					<tr><!-- First add a button to be able to add a current student as a sponsor -->
						<td colspan="6" class="center">
							Add a current student as an attendee:
							<select name="studentToAttendee" id="studentToAttendee" >
								<?php
									$query = "SELECT ID, FirstName, LastName FROM attendee WHERE SubDiscriminator = 'student' ORDER BY LastName;";
									if ( $result = $db->query($query) ) {
										while ( $obj = $result->fetch_object() ) {
											printf('<option value="%d">%s %s</option>', $obj->ID, $obj->FirstName, $obj->LastName);
										}
									} else {
										echo '<option value="NULL">There was a problem.  Please Refresh Page.</option>';
									}
								?>
							</select>
							<input type="button" value=">>" onclick="getStudentDataForSponsorForm()" />
						</td>
					</tr>
                    <!-- Row 1 -->
					<tr>
                    ...
                    <!-- Row 8 -->	
						...
						<td colspan="2">
							Y<input type="radio" name="BackgroundCheck" value="yes" <?php is_selected('BackgroundCheck', 'yes'); ?> /> &nbsp;&nbsp;&nbsp;
							N<input type="radio" name="BackgroundCheck" value="no" <?php is_selected('BackgroundCheck', 'no'); ?> />
						</td>
                    				<td align="right">Current Release Form Date</td>
						<td colspan="2">
							<?php
			...
			<!-- Row  -->	
						...
						<td class="col2" >
							Y<input type="radio" name="VanEligible" value="yes" <?php is_selected('VanEligible', 'yes'); ?> /> &nbsp;&nbsp;&nbsp;
							N<input type="radio" name="VanEligible" value="no" <?php is_selected('VanEligible', 'no'); ?> />
						</td>
						...
						<td class="col4" >
							married<input type="radio" name="MaritalStatus" value="married" <?php is_selected('MaritalStatus', 'married'); ?> /> &nbsp;&nbsp;&nbsp;
							single<input type="radio" name="MaritalStatus" value="single" <?php is_selected('MaritalStatus', 'single'); ?> /> &nbsp;&nbsp;&nbsp;
							engaged<input type="radio" name="MaritalStatus" value="engaged" <?php is_selected('MaritalStatus', 'engaged'); ?> /> &nbsp;&nbsp;&nbsp;
							divorced<input type="radio" name="MaritalStatus" value="divorced" <?php is_selected('MaritalStatus', 'divorced'); ?> />
						</td>
						
						<td class="col5">Anniversary</td>
						<td class="col6">
							<?php
								$a = get_month_init_values('dateAnniversaryMonth', 'dateAnniversaryDay', 'dateAnniversaryYear');
								
								selectBox("dateAnniversaryMonth", "dateAnniversaryMonth", "range", "1, 12", "not_set", $a[0]);
								selectBox("dateAnniversaryDay", "dateAnniversaryDay", "range", "1, 31", "not_set", $a[1]);
								$current_year = date("Y");
								selectBox("dateAnniversaryYear", "dateAnniversaryYear", "range", "$current_year, 1945", "not_set", $a[2]);
							?>
						</td>
					</tr>
